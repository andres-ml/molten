var Commander = (function() {
    ...
    // regex to parse calls to commands
    var command_call_regex = /[^ ]+|"[^"]+"/g;
    ...
    Commander.prototype.run = function(command_string) {
        var parts = command_string.match(command_call_regex);
        return this._runCommand(parts, command_string);
    };
    ...
    Commander.prototype._insertCommandInTree = function(command, callback, index, commandTree) {
        if (index < command.parts.length) {
            if (!(command.parts[index] in commandTree)) {
                commandTree[command.parts[index]] = {};
            }
            return this._insertCommandInTree(command, callback, index + 1, commandTree[command.parts[index]]);
        }
        if ("_call" in commandTree) {
            throw new Error("Command " + command.parts.join(' ') + " already defined");
        }
        commandTree._call = this._buildCall(callback, command);
    };

    Commander.prototype._buildCall = function(callback, command) {
        return function(command_arguments, command_call) {
            return callback(command.buildData(command_arguments), command_call);
        };
    };

    Commander.prototype._runCommand = function(parts, command_string) {
        var index = 0;
        ...
        return commandTree._call(parts.slice(index), command_string);
    };
